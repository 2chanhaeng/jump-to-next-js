...
요약하면 비밀번호를 사용한 로그인은 보안상 위험하고, 그렇기 떄문에 일부러 비밀번호를 이용한 로그인을 막아두었다, 즉 구현해 두지 않았다고 쓰여있다.  
...
이제 다시 `app/auth/[...nextauth]/route.ts` 로 돌아가 로그인을 구현해보자.  
